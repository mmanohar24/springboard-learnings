...
function MissionControl() {
    ...
    const [missions, setMissions] = useState(MissionItems);
    const [filter, setFilter] = useState("All");

    function updateMissionStatus(id, newStaus) {

        const updatedMissions = missions.map(
            mission => mission.id === id ? { ...mission, status: newStaus } : mission
        );
