...
SELECT COUNT(*) AS number_of_apps FROM analytics 
WHERE category = 'Family';

-- 4. Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT app_name, MAX(reviews) AS max_reviews FROM analytics 
GROUP BY app_name ORDER BY max_reviews DESC LIMIT 5;

-- 5. Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, MAX(reviews) AS max_reviews FROM analytics WHERE rating >= 4.8 GROUP BY app_name;

-- 6. Find the average rating for each category ordered by the highest rated to lowest rated.
SELECT category, AVG(rating) AS avg_category_rating FROM analytics
GROUP BY category ORDER BY avg_category_rating DESC;
